...
export function SocialProof({ blok }: { blok: any }) {
    ...
    const testimonials = blok?.testimonials || defaultTestimonials;

    return (
        <section {...(blok ? storyblokEditable(blok) : {})} className="py-24 bg-mint/10">
            <div className="container mx-auto px-4 text-center">
                <h2 className="text-4xl font-bold text-deep-purple mb-16 font-display">{blok?.headline || "Loved by families."}</h2>
...
export function TrustReassurance({ blok }: { blok: any }) {
    return (
        <section {...(blok ? storyblokEditable(blok) : {})} className="py-20 bg-white border-t border-slate-100">
            <div className="container mx-auto px-4 text-center">
...
export function FinalCta({ blok }: { blok: any }) {
    return (
        <section {...(blok ? storyblokEditable(blok) : {})} className="py-32 bg-ultraviolet text-white text-center">
            <div className="container mx-auto px-4 max-w-3xl">
                <h2 className="text-5xl font-bold mb-6 font-display">{blok?.headline || "Ready to make skincare simpler for your family?"}</h2>
                <p className="text-xl text-white/80 mb-10">{blok?.body || "Join families using Freshies to make safer, smarter skincare choices together."}</p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                    <Button size="lg" className="bg-white text-ultraviolet hover:bg-white/90 px-10 h-14 text-lg">
                        {blok?.primary_cta || "Download the app"}
                    </Button>
                    <Button size="lg" variant="outline" className="border-white text-white hover:bg-white/10 px-10 h-14 text-lg">
                        {blok?.secondary_cta || "Explore Learn"}
                    </Button>
                </div>
            </div>
        </section>
